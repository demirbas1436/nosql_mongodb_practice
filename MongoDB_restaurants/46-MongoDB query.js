//---------------------------------------------------------------------------------------------------------------------
// 46.)Write a MongoDB query to find the restaurants that have a grade with a score of 2 or a grade with a score of 6
...
db.denem2.find({
  $and: [
    {
      $or: [ // $elemMatch only works on an array.
            { borough: "Manhattan" },
            { borough: "Brooklyn" }
           ]
    },
    {
      $or: [
            { "grades.score": 2 }, // { grades: { $elemMatch: { score: 2 } } }
            { "grades.score": 6 }  // { grades: { $elemMatch: { score: 6 } } }
           ]
    },
    {
     cuisine: { $ne: "American" }
